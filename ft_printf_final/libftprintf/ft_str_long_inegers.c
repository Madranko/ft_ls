...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/29 11:18:20 by mmotov            #+#    #+#             */
/*   Updated: 2018/01/01 13:24:51 by mmotov           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char	*ft_check_size_long_int(t_form form, va_list ap)
{
	char	*str;
	...
	if (!form.size)
		...
		str = ft_itoa_base((signed long int)va_arg(ap, size_t), 10);
	return (str);
}
...
int			ft_str_long_integers(t_form *form, va_list ap)
{
	...
	if ((*form).type == 'D')
	{
		nb_s = ft_check_size_long_int(*form, ap);
